...
recommendations <http://varnomen.org/>`__.** Inferred changes with
unknown p. effects are now represented with `p.?` rather than `p.(?)`
(`#393 <https://github.com/biocommons/hgvs/issues/393/>`_).  In addition, silent SNV mutations now include the amino acid,
as in `NP_000050.2:p.Lys2597=` rather than `NP_000050.2:p.(=)` (`#317 <https://github.com/biocommons/hgvs/issues/317/>`_).
