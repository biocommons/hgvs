...
#    Grammar hash: 4999b1f0c940b666ed28f27b9db8a53a
# Parsley version: 1.3
    ...
    class Grammar(GrammarBase):
        ...
        def rule_gene_symbol(self):
            ...
            def _G_consumedby_905():
                ...
                def _G_many1_907():
                    def _G_or_908():
                        self._trace("", (12667, 12680), self.input.position)
                        _G_apply_909, lastError = self._apply(self.rule_letterOrDigit, "letterOrDigit", [])
                        self.considerError(lastError, None)
                        return (_G_apply_909, self.currentError)

                    def _G_or_910():
                        def _G_or_911():
                            self._trace("", (12684, 12687), self.input.position)
                            _G_exactly_912, lastError = self.exactly("-")
                            self.considerError(lastError, None)
                            return (_G_exactly_912, self.currentError)

                        def _G_or_913():
                            self._trace("", (12688, 12691), self.input.position)
                            _G_exactly_914, lastError = self.exactly("_")
                            self.considerError(lastError, None)
                            return (_G_exactly_914, self.currentError)

                        _G_or_915, lastError = self._or([_G_or_911, _G_or_913])
                        self.considerError(lastError, None)

                        def _G_lookahead_916():
                            self._trace("", (12695, 12708), self.input.position)
                            _G_apply_917, lastError = self._apply(self.rule_letterOrDigit, "letterOrDigit", [])
                            self.considerError(lastError, None)
                            return (_G_apply_917, self.currentError)

                        _G_lookahead_918, lastError = self.lookahead(_G_lookahead_916)
                        self.considerError(lastError, None)
                        return (_G_lookahead_918, self.currentError)

                    _G_or_919, lastError = self._or([_G_or_908, _G_or_910])
                    self.considerError(lastError, None)
                    return (_G_or_919, self.currentError)

                _G_many1_920, lastError = self.many(_G_many1_907, _G_many1_907())
                self.considerError(lastError, None)
                return (_G_many1_920, self.currentError)

            _G_consumedby_921, lastError = self.consumedby(_G_consumedby_905)
            self.considerError(lastError, "gene_symbol")
            return (_G_consumedby_921, self.currentError)
