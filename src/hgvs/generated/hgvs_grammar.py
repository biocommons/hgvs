...
#    Grammar hash: 29e5254696dc8673e47771941d6a8e3e
# Parsley version: 1.3
...
def createParserClass(GrammarBase, ruleGlobals):
    ...
    class Grammar(GrammarBase):
        ...
        def rule_g_interval(self):
            ...
            def _G_or_487():
                self._trace('', (8396, 8411), self.input.position)
        ...
        def rule_def_g_interval(self):
            ...
            def _G_or_522():
                self._trace('', (8768, 8773), self.input.position)
                _G_apply_523, lastError = self._apply(self.rule_g_pos, "g_pos", [])
                self.considerError(lastError, None)
                _locals['start'] = _G_apply_523
                self._trace('', (8779, 8783), self.input.position)
                _G_exactly_524, lastError = self.exactly('_')
                self.considerError(lastError, None)
                self._trace('', (8783, 8789), self.input.position)
            ...
            def _G_or_528():
                self._trace('', (8835, 8840), self.input.position)
        ...
        def rule_def_m_interval(self):
            ...
            def _G_or_533():
                self._trace('', (8904, 8909), self.input.position)
                _G_apply_534, lastError = self._apply(self.rule_m_pos, "m_pos", [])
                self.considerError(lastError, None)
                _locals['start'] = _G_apply_534
                self._trace('', (8915, 8919), self.input.position)
                _G_exactly_535, lastError = self.exactly('_')
                self.considerError(lastError, None)
                self._trace('', (8919, 8925), self.input.position)
            ...
            def _G_or_538():
                self._trace('', (8971, 8976), self.input.position)
        ...
        def rule_def_p_interval(self):
            ...
            def _G_or_542():
                self._trace('', (9040, 9045), self.input.position)
                _G_apply_543, lastError = self._apply(self.rule_p_pos, "p_pos", [])
                self.considerError(lastError, None)
                _locals['start'] = _G_apply_543
                self._trace('', (9051, 9055), self.input.position)
                _G_exactly_544, lastError = self.exactly('_')
                self.considerError(lastError, None)
                self._trace('', (9055, 9061), self.input.position)
            ...
            def _G_or_547():
                self._trace('', (9107, 9112), self.input.position)
        ...
        def rule_def_r_interval(self):
            ...
            def _G_or_551():
                self._trace('', (9176, 9181), self.input.position)
                _G_apply_552, lastError = self._apply(self.rule_r_pos, "r_pos", [])
                self.considerError(lastError, None)
                _locals['start'] = _G_apply_552
                self._trace('', (9187, 9191), self.input.position)
                _G_exactly_553, lastError = self.exactly('_')
                self.considerError(lastError, None)
                self._trace('', (9191, 9197), self.input.position)
            ...
            def _G_or_556():
                self._trace('', (9243, 9248), self.input.position)
        ...
        def rule_def_c_interval(self):
            ...
            def _G_or_560():
                self._trace('', (9312, 9317), self.input.position)
                _G_apply_561, lastError = self._apply(self.rule_c_pos, "c_pos", [])
                self.considerError(lastError, None)
                _locals['start'] = _G_apply_561
                self._trace('', (9323, 9327), self.input.position)
                _G_exactly_562, lastError = self.exactly('_')
                self.considerError(lastError, None)
                self._trace('', (9327, 9333), self.input.position)
            ...
            def _G_or_566():
                self._trace('', (9389, 9394), self.input.position)
        ...
        def rule_def_n_interval(self):
            ...
            def _G_or_571():
                self._trace('', (9484, 9489), self.input.position)
                _G_apply_572, lastError = self._apply(self.rule_n_pos, "n_pos", [])
                self.considerError(lastError, None)
                _locals['start'] = _G_apply_572
                self._trace('', (9495, 9499), self.input.position)
                _G_exactly_573, lastError = self.exactly('_')
                self.considerError(lastError, None)
                self._trace('', (9499, 9505), self.input.position)
            ...
            def _G_or_576():
                self._trace('', (9561, 9566), self.input.position)
        ...
        def rule_c_pos(self):
            ...
            self._trace('', (10179, 10189), self.input.position)
        ...
        def rule_g_pos(self):
            ...
            self._trace('', (10246, 10256), self.input.position)
        ...
        def rule_m_pos(self):
            ...
            self._trace('', (10313, 10323), self.input.position)
        ...
        def rule_n_pos(self):
            ...
            self._trace('', (10380, 10390), self.input.position)
        ...
        def rule_p_pos(self):
            ...
            self._trace('', (10447, 10457), self.input.position)
        ...
        def rule_r_pos(self):
            ...
            self._trace('', (10514, 10524), self.input.position)
        ...
        def rule_def_c_pos(self):
            ...
            def _G_or_613():
                self._trace('', (10607, 10612), self.input.position)
                _G_apply_614, lastError = self._apply(self.rule_base, "base", [])
                self.considerError(lastError, None)
                _locals['b'] = _G_apply_614
                self._trace('', (10614, 10621), self.input.position)
            ...
            def _G_or_618():
                self._trace('', (10706, 10710), self.input.position)
                _G_exactly_619, lastError = self.exactly('*')
                self.considerError(lastError, None)
                self._trace('', (10710, 10714), self.input.position)
                _G_apply_620, lastError = self._apply(self.rule_num, "num", [])
                self.considerError(lastError, None)
                _locals['b'] = _G_apply_620
                self._trace('', (10716, 10723), self.input.position)
        ...
        def rule_def_g_pos(self):
            ...
            def _G_or_625():
                self._trace('', (10811, 10814), self.input.position)
        ...
        def rule_def_p_pos(self):
            ...
            self._trace('', (11066, 11070), self.input.position)
        ...
        def rule_fs(self):
            ...
            self._trace('', (11570, 11575), self.input.position)
            _G_exactly_655, lastError = self.exactly('fs')
            self.considerError(lastError, 'fs')
            def _G_or_656():
                self._trace('', (11577, 11584), self.input.position)
        ...
        def rule_ext(self):
            ...
            self._trace('', (11608, 11614), self.input.position)
            _G_exactly_663, lastError = self.exactly('ext')
            self.considerError(lastError, 'ext')
            def _G_or_664():
                self._trace('', (11616, 11624), self.input.position)
        ...
        def rule_aa13_ext(self):
            ...
            def _G_or_674():
                self._trace('', (11712, 11719), self.input.position)
                _G_apply_675, lastError = self._apply(self.rule_term13, "term13", [])
                self.considerError(lastError, None)
                _locals['aat'] = _G_apply_675
                self._trace('', (11723, 11736), self.input.position)
            ...
            def _G_or_678():
                def _G_or_679():
                    self._trace('', (11763, 11767), self.input.position)
                ...
                self._trace('', (11784, 11789), self.input.position)
        ...
        def rule_dna_seq(self):
            ...
            def _G_consumedby_694():
                def _G_many1_695():
                    self._trace('', (11871, 11874), self.input.position)
        ...
        def rule_rna_seq(self):
            ...
            def _G_consumedby_699():
                def _G_many1_700():
                    self._trace('', (11888, 11891), self.input.position)
        ...
        def rule_aat13_seq(self):
            ...
            def _G_or_704():
                def _G_consumedby_705():
                    self._trace('', (11907, 11915), self.input.position)
            ...
            def _G_or_708():
                def _G_consumedby_709():
                    self._trace('', (11920, 11928), self.input.position)
        ...
        def rule_aat1_seq(self):
            ...
            def _G_or_713():
                def _G_consumedby_714():
                    self._trace('', (11942, 11947), self.input.position)
            ...
            def _G_or_717():
                def _G_consumedby_718():
                    def _G_many1_719():
                        self._trace('', (11952, 11955), self.input.position)
                    ...
                    self.considerError(lastError, None)
                    def _G_optional_722():
                        self._trace('', (11956, 11962), self.input.position)
        ...
        def rule_aat3_seq(self):
            ...
            def _G_or_728():
                def _G_consumedby_729():
                    self._trace('', (11977, 11982), self.input.position)
            ...
            def _G_or_732():
                def _G_consumedby_733():
                    def _G_many1_734():
                        self._trace('', (11987, 11990), self.input.position)
                    ...
                    self.considerError(lastError, None)
                    def _G_optional_737():
                        self._trace('', (11991, 11997), self.input.position)
        ...
        def rule_aa13_seq(self):
            ...
            def _G_or_743():
                def _G_consumedby_744():
                    self._trace('', (12012, 12019), self.input.position)
            ...
            def _G_or_747():
                def _G_consumedby_748():
                    self._trace('', (12024, 12031), self.input.position)
        ...
        def rule_aa1_seq(self):
            ...
            def _G_consumedby_752():
                def _G_many1_753():
                    self._trace('', (12044, 12047), self.input.position)
        ...
        def rule_aa3_seq(self):
            ...
            def _G_consumedby_757():
                def _G_many1_758():
                    self._trace('', (12061, 12064), self.input.position)
        ...
        def rule_aa3(self):
            ...
            def _G_or_774():
                self._trace('', (12161, 12167), self.input.position)
        ...
        def rule_aat13(self):
            ...
            def _G_or_828():
                self._trace('', (12340, 12345), self.input.position)
        ...
        def rule_aat3(self):
            ...
            def _G_or_833():
                self._trace('', (12359, 12365), self.input.position)
        ...
        def rule_dna(self):
            ...
            self._trace('', (12377, 12387), self.input.position)
        ...
        def rule_rna(self):
            ...
            self._trace('', (12516, 12526), self.input.position)
        ...
        def rule_term3(self):
            ...
            self._trace('', (12647, 12653), self.input.position)
        ...
        def rule_base(self):
            ...
            self._trace('', (12737, 12742), self.input.position)
        ...
        def rule_nnum(self):
            ...
            def _G_consumedby_871():
                self._trace('', (12751, 12754), self.input.position)
                _G_exactly_872, lastError = self.exactly('-')
                self.considerError(lastError, None)
                self._trace('', (12754, 12758), self.input.position)
        ...
        def rule_num(self):
            ...
            def _G_consumedby_877():
                def _G_many1_878():
                    self._trace('', (12779, 12784), self.input.position)
        ...
        def rule_offset(self):
            ...
            def _G_or_883():
                self._trace('', (12807, 12812), self.input.position)
        ...
        def rule_snum(self):
            ...
            def _G_consumedby_894():
                def _G_optional_895():
                    self._trace('', (12856, 12858), self.input.position)
        ...
        def rule_accn(self):
            ...
            def _G_consumedby_902():
                    ...
                    def _G_many1_905():
                        ...
                        def _G_or_908():
                            ...
                            def _G_lookahead_914():
                                self._trace('', (13004, 13017), self.input.position)
                ...
                self.considerError(lastError, None)
                def _G_optional_921():
                    self._trace('', (13023, 13026), self.input.position)
                    _G_exactly_922, lastError = self.exactly('.')
                    self.considerError(lastError, None)
                    def _G_many1_923():
                        self._trace('', (13026, 13032), self.input.position)
        ...
        def rule_opt_gene_expr(self):
            ...
            def _G_or_929():
                self._trace('', (13054, 13064), self.input.position)
                ...
                def _G_many1_943():
                    ...
                    def _G_or_946():
                        ...
                        def _G_lookahead_952():
                            self._trace('', (13189, 13202), self.input.position)
        ...
        _G_expr_530 = compile('hgvs.location.Interval(start,None)', '<string>', 'eval')
