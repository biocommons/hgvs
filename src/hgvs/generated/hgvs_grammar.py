...
#    Grammar hash: e84f0c08f3981f13aa7426baf87ad45d
# Parsley version: 1.3
...
def createParserClass(GrammarBase, ruleGlobals):
    ...
    class Grammar(GrammarBase):
        ...
        def rule_def_n_interval(self):
            ...
            def _G_or_566():
                self._trace("", (9432, 9437), self.input.position)
                _G_apply_567, lastError = self._apply(self.rule_n_pos, "n_pos", [])
                self.considerError(lastError, None)
                _locals["start"] = _G_apply_567
                self._trace("", (9443, 9447), self.input.position)
                _G_exactly_568, lastError = self.exactly("_")
                self.considerError(lastError, None)
                self._trace("", (9447, 9453), self.input.position)
            ...
            def _G_or_571():
                self._trace("", (9509, 9514), self.input.position)
        ...
        def rule_uncertain_g_interval(self):
            ...
            def _G_or_575():
                ...
                self.considerError(lastError, None)
                self._trace("", (9633, 9648), self.input.position)
                _G_apply_577, lastError = self._apply(
                    self.rule_def_g_interval, "def_g_interval", []
                )
                self.considerError(lastError, None)
                _locals["ivl_start"] = _G_apply_577
                self._trace("", (9658, 9662), self.input.position)
                _G_exactly_578, lastError = self.exactly(")")
                self.considerError(lastError, None)
                self._trace("", (9662, 9666), self.input.position)
                _G_exactly_579, lastError = self.exactly("_")
                self.considerError(lastError, None)
                self._trace("", (9666, 9670), self.input.position)
                _G_exactly_580, lastError = self.exactly("(")
                self.considerError(lastError, None)
                self._trace("", (9670, 9685), self.input.position)
                _G_apply_581, lastError = self._apply(
                    self.rule_def_g_interval, "def_g_interval", []
                )
                self.considerError(lastError, None)
                _locals["ivl_end"] = _G_apply_581
                self._trace("", (9693, 9697), self.input.position)
            ...
            def _G_or_585():
                self._trace("", (9813, 9828), self.input.position)
                _G_apply_586, lastError = self._apply(
                    self.rule_def_g_interval, "def_g_interval", []
                )
                self.considerError(lastError, None)
                _locals["ivl_start"] = _G_apply_586
                self._trace("", (9838, 9842), self.input.position)
                _G_exactly_587, lastError = self.exactly("_")
                self.considerError(lastError, None)
                self._trace("", (9842, 9846), self.input.position)
                _G_exactly_588, lastError = self.exactly("(")
                self.considerError(lastError, None)
                self._trace("", (9846, 9861), self.input.position)
                _G_apply_589, lastError = self._apply(
                    self.rule_def_g_interval, "def_g_interval", []
                )
                self.considerError(lastError, None)
                _locals["ivl_end"] = _G_apply_589
                self._trace("", (9869, 9873), self.input.position)
            ...
            def _G_or_593():
                self._trace("", (9972, 9976), self.input.position)
                _G_exactly_594, lastError = self.exactly("(")
                self.considerError(lastError, None)
                self._trace("", (9976, 9991), self.input.position)
                _G_apply_595, lastError = self._apply(
                    self.rule_def_g_interval, "def_g_interval", []
                )
                self.considerError(lastError, None)
                _locals["ivl_start"] = _G_apply_595
                self._trace("", (10001, 10005), self.input.position)
                _G_exactly_596, lastError = self.exactly(")")
                self.considerError(lastError, None)
                self._trace("", (10005, 10009), self.input.position)
                _G_exactly_597, lastError = self.exactly("_")
                self.considerError(lastError, None)
                self._trace("", (10009, 10024), self.input.position)
                _G_apply_598, lastError = self._apply(
                    self.rule_def_g_interval, "def_g_interval", []
                )
                self.considerError(lastError, None)
                _locals["ivl_end"] = _G_apply_598
            ...
            def _G_or_601():
                self._trace("", (10134, 10137), self.input.position)
                _G_exactly_602, lastError = self.exactly("(")
                self.considerError(lastError, None)
                self._trace("", (10137, 10152), self.input.position)
                _G_apply_603, lastError = self._apply(
                    self.rule_def_g_interval, "def_g_interval", []
                )
                self.considerError(lastError, None)
                _locals["iv"] = _G_apply_603
                self._trace("", (10155, 10159), self.input.position)
        ...
        def rule_c_pos(self):
            ...
            self._trace("", (10206, 10216), self.input.position)
        ...
        def rule_g_pos(self):
            ...
            self._trace("", (10273, 10283), self.input.position)
        ...
        def rule_m_pos(self):
            ...
            self._trace("", (10340, 10350), self.input.position)
        ...
        def rule_n_pos(self):
            ...
            self._trace("", (10407, 10417), self.input.position)
        ...
        def rule_p_pos(self):
            ...
            self._trace("", (10474, 10484), self.input.position)
        ...
        def rule_r_pos(self):
            ...
            self._trace("", (10541, 10551), self.input.position)
        ...
        def rule_def_c_pos(self):
            ...
            def _G_or_613():
                self._trace("", (10634, 10639), self.input.position)
                _G_apply_614, lastError = self._apply(self.rule_base, "base", [])
                self.considerError(lastError, None)
                _locals["b"] = _G_apply_614
                self._trace("", (10641, 10648), self.input.position)
            ...
            def _G_or_618():
                self._trace("", (10733, 10737), self.input.position)
                _G_exactly_619, lastError = self.exactly("*")
                self.considerError(lastError, None)
                self._trace("", (10737, 10741), self.input.position)
                _G_apply_620, lastError = self._apply(self.rule_num, "num", [])
                self.considerError(lastError, None)
                _locals["b"] = _G_apply_620
                self._trace("", (10743, 10750), self.input.position)
        ...
        def rule_def_p_pos(self):
            ...
            self._trace("", (11093, 11097), self.input.position)
        ...
        def rule_fs(self):
            ...
            self._trace("", (11597, 11602), self.input.position)
            _G_exactly_655, lastError = self.exactly("fs")
            self.considerError(lastError, "fs")

            def _G_or_656():
                self._trace("", (11604, 11611), self.input.position)
        ...
        def rule_ext(self):
            ...
            self._trace("", (11635, 11641), self.input.position)
            _G_exactly_663, lastError = self.exactly("ext")
            self.considerError(lastError, "ext")

            def _G_or_664():
                self._trace("", (11643, 11651), self.input.position)
        ...
        def rule_aa13_ext(self):
            ...
            def _G_or_674():
                self._trace("", (11739, 11746), self.input.position)
                _G_apply_675, lastError = self._apply(self.rule_term13, "term13", [])
                self.considerError(lastError, None)
                _locals["aat"] = _G_apply_675
                self._trace("", (11750, 11763), self.input.position)
            ...
            def _G_or_678():
                def _G_or_679():
                    self._trace("", (11790, 11794), self.input.position)
                ...
                self._trace("", (11811, 11816), self.input.position)
        ...
        def rule_fsext_offset(self):
            ...
            def _G_or_689():
                self._trace("", (11850, 11853), self.input.position)
        ...
        def rule_dna_seq(self):
            ...
            def _G_consumedby_694():
                def _G_many1_695():
                    self._trace("", (11898, 11901), self.input.position)
        ...
        def rule_rna_seq(self):
            ...
            def _G_consumedby_699():
                def _G_many1_700():
                    self._trace("", (11915, 11918), self.input.position)
        ...
        def rule_aat13_seq(self):
            ...
            def _G_or_704():
                def _G_consumedby_705():
                    self._trace("", (11934, 11942), self.input.position)
            ...
            def _G_or_708():
                def _G_consumedby_709():
                    self._trace("", (11947, 11955), self.input.position)
        ...
        def rule_aat1_seq(self):
            ...
            def _G_or_713():
                def _G_consumedby_714():
                    self._trace("", (11969, 11974), self.input.position)
            ...
            def _G_or_717():
                def _G_consumedby_718():
                    def _G_many1_719():
                        self._trace("", (11979, 11982), self.input.position)
                    ...
                    def _G_optional_722():
                        self._trace("", (11983, 11989), self.input.position)
        ...
        def rule_aat3_seq(self):
            ...
            def _G_or_728():
                def _G_consumedby_729():
                    self._trace("", (12004, 12009), self.input.position)
            ...
            def _G_or_732():
                def _G_consumedby_733():
                    def _G_many1_734():
                        self._trace("", (12014, 12017), self.input.position)
                    ...
                    def _G_optional_737():
                        self._trace("", (12018, 12024), self.input.position)
        ...
        def rule_aa13_seq(self):
            ...
            def _G_or_743():
                def _G_consumedby_744():
                    self._trace("", (12039, 12046), self.input.position)
            ...
            def _G_or_747():
                def _G_consumedby_748():
                    self._trace("", (12051, 12058), self.input.position)
        ...
        def rule_aa1_seq(self):
            ...
            def _G_consumedby_752():
                def _G_many1_753():
                    self._trace("", (12071, 12074), self.input.position)
        ...
        def rule_aa3_seq(self):
            ...
            def _G_consumedby_757():
                def _G_many1_758():
                    self._trace("", (12088, 12091), self.input.position)
        ...
        def rule_dna(self):
            ...
            self._trace("", (12404, 12414), self.input.position)
        ...
        def rule_rna(self):
            ...
            self._trace("", (12543, 12553), self.input.position)
        ...
        def rule_term3(self):
            ...
            self._trace("", (12674, 12680), self.input.position)
        ...
        def rule_base(self):
            ...
            self._trace("", (12764, 12769), self.input.position)
        ...
        def rule_nnum(self):
            ...
            def _G_consumedby_871():
                self._trace("", (12778, 12781), self.input.position)
                _G_exactly_872, lastError = self.exactly("-")
                self.considerError(lastError, None)
                self._trace("", (12781, 12785), self.input.position)
        ...
        def rule_num(self):
            ...
            def _G_consumedby_877():
                def _G_many1_878():
                    self._trace("", (12806, 12811), self.input.position)
        ...
        def rule_offset(self):
            ...
            def _G_or_883():
                self._trace("", (12834, 12839), self.input.position)
        ...
        def rule_snum(self):
            ...
            def _G_consumedby_894():
                def _G_optional_895():
                    self._trace("", (12883, 12885), self.input.position)
                ...
                self.considerError(lastError, None)
                self._trace("", (12886, 12890), self.input.position)
        ...
        def rule_accn(self):
            ...
            def _G_consumedby_902():
                self._trace("", (12994, 13000), self.input.position)
                ...
                def _G_optional_904():
                    def _G_many1_905():
                        def _G_or_906():
                            self._trace("", (13003, 13016), self.input.position)
                        ...
                        def _G_or_908():
                            def _G_or_909():
                                self._trace("", (13020, 13023), self.input.position)
                                _G_exactly_910, lastError = self.exactly("-")
                                self.considerError(lastError, None)
                                return (_G_exactly_910, self.currentError)

                            def _G_or_911():
                                self._trace("", (13024, 13027), self.input.position)
                            ...
                            def _G_lookahead_914():
                                self._trace("", (13031, 13044), self.input.position)
                ...
                def _G_optional_921():
                    self._trace("", (13050, 13053), self.input.position)
                    _G_exactly_922, lastError = self.exactly(".")
                    self.considerError(lastError, None)

                    def _G_many1_923():
                        self._trace("", (13053, 13059), self.input.position)
        ...
        def rule_opt_gene_expr(self):
            ...
            def _G_or_929():
                self._trace("", (13081, 13091), self.input.position)
        ...
        def rule_gene_symbol(self):
            ...
            def _G_consumedby_941():
                self._trace("", (13180, 13186), self.input.position)
                _G_apply_942, lastError = self._apply(self.rule_letter, "letter", [])
                self.considerError(lastError, None)

                def _G_many1_943():
                    def _G_or_944():
                        self._trace("", (13188, 13201), self.input.position)
                    ...
                    def _G_or_946():
                        def _G_or_947():
                            self._trace("", (13205, 13208), self.input.position)
                            _G_exactly_948, lastError = self.exactly("-")
                            self.considerError(lastError, None)
                            return (_G_exactly_948, self.currentError)

                        def _G_or_949():
                            self._trace("", (13209, 13212), self.input.position)
                        ...
                        def _G_lookahead_952():
                            self._trace("", (13216, 13229), self.input.position)
        ...
        _G_expr_563 = compile(
            "hgvs.location.BaseOffsetInterval(start,copy.deepcopy(start))",
            "<string>",
            "eval",
        )
        _G_expr_583 = compile(
            "hgvs.location.Interval(start=ivl_start._set_uncertain(), end=ivl_end._set_uncertain())",
            "<string>",
            "eval",
        )
        _G_expr_591 = compile(
            "hgvs.location.Interval(start=ivl_start, end=ivl_end._set_uncertain())",
            "<string>",
            "eval",
        )
        _G_expr_599 = compile(
            "hgvs.location.Interval(start=ivl_start._set_uncertain(), end=ivl_end)",
            "<string>",
            "eval",
        )
        _G_expr_616 = compile(
