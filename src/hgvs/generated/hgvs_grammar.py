...
#    Grammar hash: e3ee2b1642d68d44a9f99980cd0c6df4
# Parsley version: 1.3
    ...
    class Grammar(GrammarBase):
        ...
        def rule_gene_symbol(self):
            ...
            def _G_consumedby_905():
                self._trace("", (12702, 12708), self.input.position)
                _G_apply_906, lastError = self._apply(self.rule_letter, "letter", [])
                self.considerError(lastError, None)

                def _G_many1_907():
                    def _G_or_908():
                        self._trace("", (12710, 12713), self.input.position)
                        _G_exactly_909, lastError = self.exactly("_")
                        self.considerError(lastError, None)
                        return (_G_exactly_909, self.currentError)

                    def _G_or_910():
                        self._trace("", (12715, 12719), self.input.position)
                        _G_exactly_911, lastError = self.exactly("-")
                        self.considerError(lastError, None)
                        return (_G_exactly_911, self.currentError)

                    def _G_or_912():
                        self._trace("", (12721, 12735), self.input.position)
                        _G_apply_913, lastError = self._apply(self.rule_letterOrDigit, "letterOrDigit", [])
                        self.considerError(lastError, None)
                        return (_G_apply_913, self.currentError)

                    _G_or_914, lastError = self._or([_G_or_908, _G_or_910, _G_or_912])
                    self.considerError(lastError, None)
                    return (_G_or_914, self.currentError)

                _G_many1_915, lastError = self.many(_G_many1_907, _G_many1_907())
                self.considerError(lastError, None)
                return (_G_many1_915, self.currentError)

            _G_consumedby_916, lastError = self.consumedby(_G_consumedby_905)
            self.considerError(lastError, "gene_symbol")
            return (_G_consumedby_916, self.currentError)
        ...
        _G_expr_898 = compile("hgvs.sequencevariant.validate_gene_symbol(gene)", "<string>", "eval")
