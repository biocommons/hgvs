...
class TestParseAllCvids(unittest.TestCase):
    longMessage = True
    ...
    def test_parse_hgvsg_cvids(self):
        key = 'HGVSg'
        omissions = [r'\(\d+\)$', r'copy\d+$', r'dup\d+$']
        self._parse_all_cvids(key, omissions)

    def test_parse_hgvsc_cvids(self):
        key = 'HGVSc'
        omissions = [r'copy\d+$', r'dup\d+$']
        self._parse_all_cvids(key, omissions)

    def test_parse_hgvsp_cvids(self):
        key = 'HGVSp'
    ...
    def _parse_all_cvids(self, key, omissions = []):
        ...
        outfilename = 'cvids_all_parsed_{}.out'.format(key)
