...
class UTA1_Base(object):

    def test_get_tx_exons(self):    
        tx_exons = self.uta1.get_tx_exons('NM_000551.3','NC_000003.11', 'splign')
        self.assertEqual( 3, len(tx_exons) )

    def test_get_tx_info(self):
        tx_info = self.uta1.get_tx_info('NM_000051.3', 'AC_000143.1', 'splign')
    ...
    def test_get_tx_mapping_options(self):
        tx_mapping_options = self.uta1.get_tx_mapping_options('NM_000551.3')
    ...
    def test_get_tx_seq(self):
        tsi = self.uta1.get_tx_seq('NM_000551.3')
        seq = tsi
    ...
    def test_get_tx_for_gene(self):
        tig = self.uta1.get_tx_for_gene('VHL')
        self.assertEqual( 11, len(tig) )

    def test_get_tx_exons_invalid_tx_ac(self):
        tx_exons = self.uta1.get_tx_exons('NM_999999.9','NC_000003.11', 'splign')
        self.assertIsNone( tx_exons )

    def test_get_tx_exons_invalid_alt_ac(self):
        tx_exons = self.uta1.get_tx_exons('NM_000551.3','NC_000999.9', 'splign')
        self.assertIsNone( tx_exons )

    def test_get_tx_exons_invalid_alt_aln_method(self):
        tx_exons = self.uta1.get_tx_exons('NM_000551.3','NC_000999.9', 'best')
        self.assertIsNone( tx_exons )

    def test_get_tx_info_invalid_tx_ac(self):
        tx_info = self.uta1.get_tx_info('NM_999999.9', 'AC_000143.1', 'splign')
        self.assertIsNone( tx_info )

    def test_get_tx_mapping_options_invalid(self):
        tx_info_options = self.uta1.get_tx_mapping_options('NM_999999.9')
        self.assertIsNone( tx_info_options )

    def test_get_tx_seq_invalid_ac(self):
        tsi = self.uta1.get_tx_seq('NM_999999.9')
        self.assertIsNone( tsi )

    def test_get_tx_for_gene_invalid_gene(self):
        tig = self.uta1.get_tx_for_gene('GENE')
    ...
    def test_get_acs_for_protein_seq(self):
        ...
        """
        s = re.sub('\s+','',s.upper())
        self.assertEqual( sorted(self.uta1.get_acs_for_protein_seq(s)), sorted(exp) )
    ...
    def test_get_gene_info(self):
        gene_info = self.uta1.get_gene_info('VHL')
        self.assertEqual( 'VHL', gene_info['hgnc'])
        self.assertEqual( '3p25.3', gene_info['maploc'])
        self.assertEqual( 6, len(gene_info))


class Test_bdi_sources_uta1_UTA1_default(unittest.TestCase, UTA1_Base):
    def setUp(self):
        self.uta1 = hgvs.dataproviders.uta.connect()
        return self
